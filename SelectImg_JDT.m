...
% NUM = length(nu) + length(img) + length(kodim);disp(NUM);
for ss = 1:length(scale_list)
    scale = scale_list(ss);
    imgRoot_info = sprintf('../Reference JDT_PQPR/Results/0508/%d/', scale);
    ...
    for cc = 1:length(nu)
        ...
        for ii=1:length(imgs_names)
            if  ~strcmp(imgs_names(ii).name(1,1:3) ,'Img') && strcmp(imgs_names(ii).name(1,1:2), target) 
                ...
                str = sprintf('../Reference JDT_PQPR/Results/0509/%d/%s', scale, imgs_names(ii).name);
                imwrite(I, str); 
                break;
            end
        end
